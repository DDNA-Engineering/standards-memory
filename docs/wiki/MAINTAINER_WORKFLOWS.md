...
An exact `ambiguous-numbered` unsupported outline record may also be exported singly. Its 0.2.0 draft retains `source_candidate_kind: "unsupported_region"`, `classification_state: "pending_explicit_reviewer_kind"`, and `proposed_node.kind: "unsupported_region"`; it does not assert a clause. The reviewer must inspect the page and explicitly choose a supported structural `node.kind` and a source-accurate `clause_reference` different from the synthetic unsupported-region locator before promotion. A numeric table row should remain unsupported if no such decision is justified. Other unsupported regions and pending-classification candidates in shard export remain rejected. The decision JSON stays at 0.1.0 because it binds the exact draft bytes and requires the same complete review provenance.

...
class QueryCacheAndBatchTests(unittest.TestCase):
    ...
    def test_search_modes_are_explicit_distinct_and_cache_separated(self) -> None:
        ...
        explicit_all = self.service.search("axial ingress", "local-user", query_mode="all_terms")
        any_terms = self.service.search("axial ingress", "local-user", query_mode="any_terms")
        exact_phrase = self.service.search("steady axial load", "local-user", query_mode="exact_phrase")
        reversed_phrase = self.service.search("axial steady load", "local-user", query_mode="exact_phrase")
        ...
        self.assertEqual(["clause-4.2.1"], [item["record_id"] for item in exact_phrase["results"]])
        ...
        self.assertEqual(
            {"mode": "any_terms", "normalized_query": "axial ingress", "parsed_terms": ["axial", "ingress"]},
            any_terms["query_interpretation"],
        )

        identifier_chunks = self.service.search("4.2.1 water", "local-user", query_mode="any_terms")
        self.assertEqual(
            {"clause-4.2.1", "note-4.2.1-1", "clause-4.2.2"},
    ...
    def test_natural_language_preserves_modality_conditions_and_request_echo_on_cache_hit(self) -> None:
        ...
        first = self.service.search(
            "how shall connector be retained under load", "local-user", query_mode="natural_language"
        )
        second = self.service.search(
            "what shall connector be retained under load", "local-user", query_mode="natural_language"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            second["query_interpretation"]["normalized_query"],
        )
        self.assertEqual(
            first["query_interpretation"]["selected_strategy"],
            second["query_interpretation"]["selected_strategy"],
        )
    ...
    def test_natural_language_rejects_scaffolding_only_without_store_access(self) -> None:
        with patch.object(self.service.store, "search", wraps=self.service.store.search) as lexical_search:
            with self.assertRaises(StandardsForgeError) as caught:
                self.service.search(
                    "what does the standard say", "local-user", query_mode="natural_language"
                )
        self.assertEqual("invalid_query", caught.exception.code)
        lexical_search.assert_not_called()
    ...
    def test_natural_language_prefixes_identifiers_but_not_quantities(self) -> None:
        ...
        try:
            ...
            connection.close()
        identifier = self.service.search("MIL-STD-810", "local-user", query_mode="natural_language")

...
class StandardsForgeService:
    ...
    def list_documents(
        ...
        if cursor is not None:
            ...
            require(
                payload["normalized_identifier_prefix"] == normalized_prefix,
                "cursor_scope_mismatch",
                "The cursor belongs to a different identifier prefix.",
            )
            require(
                payload["visibility_fingerprint"] == visibility_fingerprint,
                "cursor_inventory_changed",
                "The authorized document inventory changed after the cursor was issued.",
            )
    ...
    def _resolve_candidates(
        ...
        for row in related_rows:
            if row in exact_rows:
                match_basis = "exact_identifier_alternate_selector"
            elif row in prefix_rows:
                match_basis = "identifier_prefix"
            else:
                match_basis = "document_family"
            candidates.append(
                {
    ...
    def resolve_document(
        ...
        if not rows:
            ...
            require(
                before_visibility == candidate_visibility,
                "authorization_changed",
                "Document visibility changed while the resolution result was being assembled; retry the query.",
            )
            candidates, candidates_truncated = self._resolve_candidates(
                authorized_rows, normalized, edition_id, representation
            )
            try:
            ...
            require(
                self.store.cache_state(principal_id)["visibility_fingerprint"] == candidate_visibility,
                "authorization_changed",
                "Document visibility changed while the resolution result was being assembled; retry the query.",
            )
            raise StandardsForgeError(
        ...
        if len(rows) != 1:
            ...
            require(
                self.store.cache_state(principal_id)["visibility_fingerprint"] == before_visibility,
                "authorization_changed",
                "Document visibility changed while the resolution result was being assembled; retry the query.",
            )
            raise StandardsForgeError(
        ...
        require(
            current["grant_policy_fingerprint"] == row["grant_policy_fingerprint"]
            and self.store.cache_state(principal_id)["visibility_fingerprint"] == before_visibility,
            "authorization_changed",
            "Document visibility changed while the resolution result was being assembled; retry the query.",
        )
        return result
    ...
    def _decode_document_cursor(self, token: str) -> dict[str, Any]:
        ...
        require(set(payload) == required_keys, "invalid_cursor", "The continuation cursor fields are invalid.")
        require(
            payload["version"] == 1 and payload["operation"] == "list_documents",
            "invalid_cursor",
            "The continuation cursor version or operation is unsupported.",
        )
        require(
            isinstance(payload["last_key"], list)
            and len(payload["last_key"]) == 5
            and all(isinstance(value, str) for value in payload["last_key"]),
            "invalid_cursor",
            "The continuation cursor document key is invalid.",
        )
        require(
            type(payload["returned_count"]) is int and payload["returned_count"] >= 0,
            "invalid_cursor",
        ...
        require(
            payload["normalized_identifier_prefix"] is None
            or isinstance(payload["normalized_identifier_prefix"], str),
            "invalid_cursor",
            "The continuation cursor prefix is invalid.",
        )
        return payload

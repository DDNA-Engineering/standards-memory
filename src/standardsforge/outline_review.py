...
MAX_REVIEW_JSON_BYTES = 8 * 1024 * 1024
...
def _read_closed_json(path: str | Path, keys: set[str] | tuple[set[str], ...], label: str) -> dict[str, Any]:
    ...
    require(len(data) <= MAX_REVIEW_JSON_BYTES, "invalid_outline_review", f"{label} exceeds the review JSON size limit.")
    try:
...
def _validate_reviewed_node(base: Any, draft: dict[str, Any], node: dict[str, Any]) -> None:
...
def export_outline_review_draft(
    ...
    return _export_outline_review_draft(
        outline_pack, source_page_pack, record_id, output_path,
    )


def _export_outline_review_draft(
    outline_pack: str | Path,
    source_page_pack: str | Path,
    record_id: str,
    output_path: str | Path,
    *,
    validated_base: Any = None,
    validated_outline: Any = None,
) -> dict[str, Any]:
    """Internal fast path for a batch that already holds both validated packs open."""

    require((validated_base is None) == (validated_outline is None), "invalid_outline_review", "Both validated inputs must be supplied together.")

    output = Path(output_path).resolve()
    require(not output.exists(), "compiler_output_exists", "The outline review draft already exists.", path=str(output))
    with (
        nullcontext(validated_base) if validated_base is not None else open_validated_pack(source_page_pack)
    ) as base, (
        nullcontext(validated_outline) if validated_outline is not None else open_validated_pack(outline_pack)
    ) as outline:
